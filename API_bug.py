"""
new Env('一个小白修改过检测更新cookie的bug测试脚本');
"""
...
def login(username, password):
    url = "http://127.0.0.1:5678/api/login?t=%s" % gettimestamp()
...
def getitem(key):
    url = "http://127.0.0.1:5000/api/envs?searchValue=%s&t=%s" % (key, gettimestamp())
    r = s.get(url)
...
def getckitem(key):
    url = "http://127.0.0.1:5000/api/envs?searchValue=JD_COOKIE&t=%s" % gettimestamp()
    r = s.get(url)
